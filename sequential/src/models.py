...
class MLPBERT4Rec(nn.Module):
    ...
    def forward(self, log_seqs, labels):
        ...
        if self.mode == "all":
            ...
            out_in = self.MLP(mlp_in)
        elif self.mode == "bert":
            out_in = bert_out
        elif self.mode == "mlp":
            out_in = self.MLP(mlp_merge)
            # TODO: or `out_in = self.MLP(mlp_merge*mlp_mask)` ?
